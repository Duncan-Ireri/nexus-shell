...
func TestLoadIgnoresInvalidConfig(t *testing.T) {
	...
	require.NoError(t, fs.MkdirAll("/xdg/NexusShell", 0o755))
	require.NoError(t, afero.WriteFile(fs, "/xdg/NexusShell/registries.json", []byte("{not json"), 0o644))
	assert.Len(t, Load(fs), 1)
	...
	require.NoError(t, afero.WriteFile(fs, "/xdg/NexusShell/registries.json", []byte(entries), 0o644))
	sources := Load(fs)
