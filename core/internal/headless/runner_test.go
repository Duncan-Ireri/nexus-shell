...
func TestDepExists(t *testing.T) {
	dependencies := []deps.Dependency{
		...
		{Name: "dms (NexusShell)", Status: deps.StatusInstalled},
	...
	tests := []struct {
		...
		{"existing dep with special chars", "dms (NexusShell)", true},
...
func TestBuildDisabledItems(t *testing.T) {
	dependencies := []deps.Dependency{
		...
		{Name: "dms (NexusShell)", Status: deps.StatusInstalled, Required: true},
		{Name: "dms-greeter", Status: deps.StatusMissing},
	...
	tests := []struct {
		...
		},
		{
			...
			excludeDeps: []string{"dms (NexusShell)"},
			wantErr:     true,
