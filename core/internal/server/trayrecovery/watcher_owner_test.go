...
// dbus-run-session -- env NEXUS_TEST_SCRATCH_BUS=1 go test ./internal/server/trayrecovery/ -run TestWatchWatcherOwner -v
func TestWatchWatcherOwnerRescansOnRestart(t *testing.T) {
	if os.Getenv("NEXUS_TEST_SCRATCH_BUS") != "1" {
		t.Skip("needs a scratch session bus (NEXUS_TEST_SCRATCH_BUS=1 under dbus-run-session)")
	}
