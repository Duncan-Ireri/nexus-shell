...
var greeterCmd = &cobra.Command{
	Use:                "greeter",
	Short:              "Deprecated: moved to the standalone nexus-greeter binary",
	Long:               "The greeter has moved to the standalone nexus-greeter package.\nThis command forwards to 'nexus-greeter' when it is installed.",
	DisableFlagParsing: true,
	SilenceUsage:       true,
	RunE: func(_ *cobra.Command, args []string) error {
		binary, err := exec.LookPath("dms-greeter")
		if err != nil {
			return fmt.Errorf("'nexus greeter' has moved to the standalone dms-greeter package.\n  %s", greeterPackageInstallHint())
		}
		if isLegacyWrapperScript(binary) {
			return fmt.Errorf("'nexus greeter' has moved to the standalone dms-greeter package; %s is the old wrapper script.\n  %s", binary, greeterPackageInstallHint())
		}
		fmt.Fprintln(os.Stderr, "warning: 'nexus greeter' is deprecated; use 'dms-greeter' directly")
