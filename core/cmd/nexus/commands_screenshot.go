...
import (
	...
	"github.com/nexus-shell/nexus-shell/core/internal/clipboard"
	"github.com/nexus-shell/nexus-shell/core/internal/notify"
	"github.com/nexus-shell/nexus-shell/core/internal/screenshot"
...
Examples:
  nexus screenshot                     # Region select, save file + clipboard
  nexus screenshot full                # Full screen of focused output
  nexus screenshot all                 # All screens combined
  nexus screenshot output -o DP-1      # Specific output
  nexus screenshot window              # Focused window (Hyprland)
  nexus screenshot last                # Last region (pre-selected)
  nexus screenshot --reset             # Reset last region pre-selection
  nexus screenshot --no-clipboard      # Save file only
  nexus screenshot --no-file           # Clipboard only
  nexus screenshot --no-confirm        # Region capture on mouse release
  nexus screenshot --cursor=on         # Include cursor
  nexus screenshot -f jpg -q 85        # JPEG with quality 85
  nexus screenshot --json              # Print capture metadata as JSON
  nexus screenshot scroll              # Scroll capture, Enter finishes / Esc cancels
  nexus screenshot scroll --interval 250`,
}
