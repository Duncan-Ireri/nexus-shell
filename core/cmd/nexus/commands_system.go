...
var systemCmd = &cobra.Command{
	...
	Long:  "System-level operations (updates, etc.). Runs against installed package managers directly; does not require the nexus-shell server.",
}
...
Examples:
  nexus system update --check                  # list available updates
  nexus system update                          # apply updates (interactive prompt)
  nexus system update --noconfirm              # apply updates without prompting
  nexus system update --dry                    # simulate without changing anything
  nexus system update --no-flatpak --noconfirm # apply system updates only
  nexus system update --interval 3600          # set the server poll interval to 1h`,
	Run: runSystemUpdate,
}
