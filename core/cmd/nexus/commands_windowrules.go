...
import (
	...
	"github.com/nexus-shell/nexus-shell/core/internal/windowrules"
	"github.com/nexus-shell/nexus-shell/core/internal/windowrules/providers"
...
var windowrulesListCmd = &cobra.Command{
	...
	Long:  "List all window rules from compositor config file. Returns JSON with rules and nexus-shell status.",
...
var windowrulesAddCmd = &cobra.Command{
	...
	Short: "Add a window rule to nexus-shell file",
	Long:  "Add a new window rule to the nexus-shell-managed rules file.",
...
var windowrulesUpdateCmd = &cobra.Command{
	...
	Short: "Update a window rule in nexus-shell file",
	Long:  "Update an existing window rule in the nexus-shell-managed rules file.",
...
var windowrulesRemoveCmd = &cobra.Command{
	...
	Short: "Remove a window rule from nexus-shell file",
	Long:  "Remove a window rule from the nexus-shell-managed rules file.",
	Args:  cobra.ExactArgs(2),
	ValidArgsFunction: func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		if len(args) == 0 {
			return []string{"hyprland", "niri", "mango"}, cobra.ShellCompDirectiveNoFileComp
...
var windowrulesReorderCmd = &cobra.Command{
	...
	Short: "Reorder window rules in nexus-shell file",
