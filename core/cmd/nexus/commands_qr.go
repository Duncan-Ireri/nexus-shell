...
import (
	...
	"github.com/nexus-shell/nexus-shell/core/internal/qrcode"
...
Input:
  nexus qr "https://example.com"     # encode an argument
  echo -n "data" | nexus qr          # encode stdin
  nexus qr -                         # encode stdin explicitly

Output (combine freely):
  nexus qr "text" --clipboard        # copy PNG image to clipboard
  nexus qr "text" --copy-text        # copy the source text to clipboard
  nexus qr "text" -o code.png        # save a PNG file
  nexus qr "text" > code.png         # PNG to stdout (piped)
  nexus qr "text" --render           # force terminal render
...
WiFi:
  nexus qr wifi MySSID -p secret     # build from an explicit password
  nexus qr wifi MySSID               # pull the saved secret from the shell`,
...
credentials are fetched from the running nexus-shell shell (like the network panel).`,
