...
import (
	...
	"github.com/nexus-shell/nexus-shell/core/internal/colorpicker"
...
This is the screen eyedropper CLI. To open the in-shell color modal, use:
  nexus ipc call color-picker toggle
...
Examples:
  nexus color pick                # Pick color, output as hex
  nexus color pick --rgb          # Output as RGB
  nexus color pick --json         # Output all formats as JSON
  nexus color pick --hex -l       # Output hex in lowercase
  nexus color pick -a             # Auto-copy result to clipboard`,
	Run: runColorPick,
}
