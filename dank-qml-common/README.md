...
- `Common/Proc.qml` exposes `dmsBin` (`NEXUS_EXECUTABLE` env override) as a nexus-shell convenience; it is inert elsewhere.
- Log stays app-owned so each app keeps its own env-var prefix (`NEXUS_LOG_LEVEL`, `DANKCAL_LOG_LEVEL`, ...).
