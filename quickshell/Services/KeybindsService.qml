...
Singleton {
    ...
    function loadCheatsheet(provider) {
        ...
        cheatsheetProcess.command = ["nexus", "keybinds", "show", target];
    ...
    function loadBinds(showLoading) {
        ...
        loadProcess.command = ["nexus", "keybinds", "show", currentProvider];
    ...
    function saveBind(originalKey, bindData) {
        ...
        if (!bindData.key || !Actions.isValidAction(bindData.action))
            return;
        saving = true;
        const cmd = ["nexus", "keybinds", "set", currentProvider, bindData.key, bindData.action, "--desc", bindData.desc || ""];
    ...
    function _maybeWarnHyprlandLegacyConf() {
        ...
        if (!dmsStatus.exists || dmsStatus.included)
            return;
        _hyprlandLegacyWarnShown = true;
        ToastService.showWarning(I18n.tr("Hyprland config include missing"), I18n.tr("DMS Settings writes Lua keybinds. Add the DMS include so edits apply."), "nexus setup", "hyprland-migration");
    }

    function showHyprlandReadOnlyWarning() {
        ToastService.showWarning(I18n.tr("Hyprland conf mode"), I18n.tr("This install is still using hyprland.conf. Run nexus setup to migrate before changing these settings."), "nexus setup", "hyprland-migration");
    }
    ...
    function removeBind(key) {
        ...
        if (!key)
            return;
        removeProcess.command = ["nexus", "keybinds", "remove", currentProvider, key];
    ...
    function resetBind(key) {
        ...
        if (!key)
            return;
        removeProcess.command = ["nexus", "keybinds", "reset", currentProvider, key];
        removeProcess.running = true;
        bindRemoved(key);
    }
