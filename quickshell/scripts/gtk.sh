...
# assets: relink adw-gtk3 assets if NEXUS already manages gtk.css
MODE="${2:-apply}"
IS_LIGHT="${3:-light}"
SHELL_DIR="$4"
...
# NEXUS patches the theme css in place, which needs a user-writable copy;
	...
	for variant in light dark; do
		...
		if [ -d "$dest/gtk-3.0" ]; then
			...
			if [ "$src_sum" != "$(sed -n 's/^sha256=//p' "$marker")" ]; then
				echo "System adw-gtk3${name} changed, refreshing NEXUS copy"
				rm -rf "$dest"
			else
				continue
			fi
...
# gtk.css is NEXUS-managed if it is our symlink or carries our import line.
...
inject_dank_import() {
	...
	if [ -L "$gtk_css" ]; then
		if ! nexus_managed_css "$gtk_css"; then
			echo "Warning: '$gtk_css' is a user-managed symlink; leaving it untouched" >&2
			echo "Import dank-colors.css from your own stylesheet to use NEXUS colors" >&2
			return 1
		fi
		rm "$gtk_css"
	fi
...
remove_dank_import() {
	...
	if [ -L "$gtk_css" ]; then
		if nexus_managed_css "$gtk_css"; then
			rm "$gtk_css"
			echo "Removed NEXUS-managed symlink '$gtk_css'"
		fi
		return 0
	fi
...
remove_gtk3_patch() {
	...
	sed -i '/\/\* BEGIN NEXUS OVERRIDE \*\//,/\/\* END NEXUS OVERRIDE \*\//d' "${theme_dir}/gtk${css_variant}.css"
	return $?
}
...
apply_gtk3_colors() {
	...
	# adw-gtk3 carries the colors; ensure there's no NEXUS global override
	...
	for variant in light dark; do
		...
		for css in gtk.css gtk-dark.css; do
			if [ -f "$adw_gtk3_dir/$css" ] && [ ! -f "$adw_gtk3_dir/$css.nexus-backup" ]; then
				cp "$adw_gtk3_dir/$css" "$adw_gtk3_dir/$css.nexus-backup"
			fi
		done
	done
}
...
case "$MODE" in
	patch)
		...
		if ! nexus_managed_css "$CONFIG_DIR/gtk-4.0/gtk.css"; then
			echo "NEXUS GTK theming is not applied, skipping patch"
			exit 2
		fi
	...
	apply)
		...
		;;
	assets)
		# Repair pass at startup; only acts when NEXUS already manages gtk.css
		if nexus_managed_css "$CONFIG_DIR/gtk-3.0/gtk.css"; then
