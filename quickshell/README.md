...
QML-based desktop shell interface for NexusShell providing panels, widgets, and overlays.
...
External plugins in `~/.config/NexusShell/plugins/`:
...
Process {
    command: ["nexus", "ipc", "call", "spotlight", "toggle"]
    running: true
}
```
