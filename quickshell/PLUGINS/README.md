...
Plugins let you add custom widgets to DankBar and Control Center. They're discovered from `~/.config/NexusShell/plugins/` and managed via PluginService.
...
1. **PluginService** (`Services/PluginService.qml`)
   ...
   - Discovers plugins from `$CONFIGPATH/NexusShell/plugins/`
...
Each plugin must be a directory in `$CONFIGPATH/NexusShell/plugins/` containing:

```
$CONFIGPATH/NexusShell/plugins/YourPlugin/
...
1. Reload after editing a file: `nexus ipc call plugins reload yourPlugin`
...
// Path to plugins directory ($CONFIGPATH/NexusShell/plugins)
...
```bash
mkdir -p $CONFIGPATH/NexusShell/plugins/MyPlugin
cd $CONFIGPATH/NexusShell/plugins/MyPlugin
```
...
1. Run the shell: `qs -p $CONFIGPATH/quickshell/nexus/shell.qml`
...
Or edit `$CONFIGPATH/quickshell/nexus/config.json`:
...
Item {
    function copyToClipboard(text) {
        Quickshell.execDetached(["nexus", "cl", "copy", text])
    }
}
```
...
MouseArea {
    onClicked: {
        Quickshell.execDetached(["nexus", "cl", "copy", modelData])
...
```bash
qs -v -p $CONFIGPATH/quickshell/nexus/shell.qml
```
...
1. **Plugin Not Detected**
   ...
   - Verify directory is in `$CONFIGPATH/NexusShell/plugins/`
...
4. **Data Not Persisting**
   ...
   - Check `$CONFIGPATH/NexusShell/settings.json` for pluginSettings data
