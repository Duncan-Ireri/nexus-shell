...
Singleton {
    ...
    readonly property string stateDir: Paths.strip(StandardPaths.writableLocation(StandardPaths.GenericCacheLocation).toString()) + "/NexusShell"
    readonly property bool envDisableMatugen: Quickshell.env("NEXUS_DISABLE_MATUGEN") === "1" || Quickshell.env("NEXUS_DISABLE_MATUGEN") === "true"
    ...
    function setDesiredTheme(kind, value, isLight, iconTheme, matugenType, stockColors) {
        ...
        const args = ["nexus", "matugen", "queue", "--state-dir", stateDir, "--shell-dir", shellDir, "--config-dir", configDir, "--kind", desired.kind, "--value", desired.value, "--mode", desired.mode, "--icon-theme", desired.iconTheme, "--matugen-type", desired.matugenType,];
