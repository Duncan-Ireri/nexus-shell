...
Singleton {
    ...
    readonly property string pluginSettingsPath: _configDir + "/NexusShell/plugin_settings.json"
    ...
    function loadSettings() {
        ...
        try {
            ...
            obj = root._applyUserOverrides(obj);
    ...
    // Three-tier config layering: compiled-in defaults < settings.json (GUI-managed,
    // above) < user-overrides.json (hand-edited, always applied last, never written
    // to by the shell itself). This lets a user pin values that survive both Settings
    // UI changes and shell updates. See docs/CONFIG_LAYERING.md.
    function _applyUserOverrides(obj) {
        try {
            const txt = userOverridesFile.text();
            if (!txt || !txt.trim())
                return obj;
            const overrides = JSON.parse(txt);
            return root._deepMerge(obj || {}, overrides);
        } catch (e) {
            log.warn("Failed to parse user-overrides.json, ignoring:", e.message);
            return obj;
        }
    }
    ...
    function _deepMerge(base, overrides) {
        if (typeof overrides !== "object" || overrides === null || Array.isArray(overrides))
            return overrides;
        const result = (typeof base === "object" && base !== null && !Array.isArray(base)) ? Object.assign({}, base) : {};
        for (const key in overrides) {
            result[key] = root._deepMerge(result[key], overrides[key]);
        }
        return result;
    }

    FileView {
        id: userOverridesFile

        path: StandardPaths.writableLocation(StandardPaths.ConfigLocation) + "/NexusShell/user-overrides.json"
        blockLoading: true
        watchChanges: true
    ...
    FileView {
        ...
        path: StandardPaths.writableLocation(StandardPaths.ConfigLocation) + "/NexusShell/settings.json"
        blockLoading: true
        blockWrites: true
        atomicWrites: true
        watchChanges: true
